...
/** Mini IG-style 3-col grid preview used as thumbnail for instagram_feed projects */
function IgMiniGrid({ images, className }: { images: string[]; className?: string }) {
  const count = Math.min(images.length, 9);
  if (count === 0) return null;
  return (
    <div className={cn("grid grid-cols-3 gap-0.5 h-full w-full", className)}>
  ...
  return (
    ...
    >
      ...
      {isIgFeed && galleryUrls.length > 0 ? (
        ...
        <div className="relative aspect-[3/3] overflow-hidden bg-muted/40">
          <IgMiniGrid images={galleryUrls} />
          {/* Hover overlay */}
